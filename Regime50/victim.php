...
$cutTreeQuery = "SELECT DISTINCT cut_tree FROM damagetree50";
...
if ($cutTreeResult->num_rows > 0) {
    while ($cutTreeRow = $cutTreeResult->fetch_assoc()) {
        ...
        $dataQuery = "SELECT victim, category_damage FROM damagetree50 WHERE cut_tree = '$cutTree'";
        ...
        // Populate rows for the current Cut_Tree
        $firstRow = true;
        while ($dataRow = $dataResult->fetch_assoc()) {
            echo "<tr>";
            if ($firstRow) {
                // echo "<td rowspan='{$dataResult->num_rows}'>$cutTree</td>";
                $firstRow = false;
            }
